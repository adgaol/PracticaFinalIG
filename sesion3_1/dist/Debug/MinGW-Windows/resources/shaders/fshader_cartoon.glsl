...
void main() {
    ...
    if     (outColor.r > 0.75) outColor = vec4(0.8, 0.8, 0.8, 1.0);
    else if(outColor.r > 0.50) outColor = vec4(0.8, 0.8, 0.3, 1.0);
    else if(outColor.r > 0.25) outColor = vec4(0.2, 0.8, 0.2, 1.0);
    else                       outColor = vec4(0.8, 0.1, 0.8, 1.0);
...
vec4 funPositional(Light light, Material material, vec3 N, vec3 V) {
    ...
    vec4 color = attenuation*(ambient + diffuse + specular);
...
vec4 funFocal(Light light, Material material, vec3 N, vec3 V) {
    ...
    vec4 color = attenuation*(ambient + intensity*(diffuse + specular));
